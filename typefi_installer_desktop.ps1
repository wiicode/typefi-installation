...
<#
ABOUT THIS SCRIPT:
Installs a complete Typefi Desktop solution on a Windows Desktop.
#>
...
function go_server {
    ...
    ftp_file $manifest.server_desktop "Typefi__Server for Desktop"
    $installer_log = "$PSScriptRoot\logs\bootstrap_typefi_desktop_install.txt"
    $installer =  "$PSScriptRoot\staging\$($manifest.server_desktop)"
...
function go_designer {
    ...
    ftp_file $manifest.designer_desktop "Typefi__Designer"
    $installer_log = "$PSScriptRoot\logs\bootstrap_typefi_designer_install.txt"
    $installer =  "$PSScriptRoot\staging\$($manifest.designer_desktop)"
    $arguments = "APPDIR=$payload\Typefi /qn /l*v $installer_log"
    Start-Process $installer -ArgumentList $arguments -Verb runAs -Wait
}
...
function go_typefitter {
    write-host ("-.. . -... ..- --.DEBUG: The name of this function is: {0} " -f $MyInvocation.MyCommand)
    service-stop "InDesignServerService x64"
    ftp_file $manifest.typefitter_desktop "Typefi__Typefitter"
